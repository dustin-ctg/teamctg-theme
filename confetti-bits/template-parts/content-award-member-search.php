...
if ( isset( $_POST['team_search_submitted'] ) ) {
	if (trim($_POST['search_terms']) === '') {
		...
		if ( ! is_this_dustin() && ! is_this_admins() ) {
...
<div class="confetti-bits-module">
	<h4 class="confetti-bits-heading">
		Search for a Team Member to Award
	</h4>
	<form class="award-form" method="post" name="team_search" autocomplete="off">
		<ul class="award-form-page-section">
			...
			<li class="award-form-line">
				<div class="search-container">
					<input type="submit" class="award-submit" action="content-confetti-bits-award-form.php" value="Search">
				</div>
			</li>
		</ul>
		<input type="hidden" name="team_search_submitted" id="team_search_submitted" value="true">
	</form>
	
	<?php 	if ( $search_error != '' ) { ?>
	<span class="error"><?php echo $search_error; ?></span>
	<?php } else if ( isset ( $_POST [ "team_search_submitted" ] ) ) {
			...
			case ( count ( $members ) === 1 ): ?> 
				...
				<div class="member-selection-container">
					<?php foreach ( $members as $member ) { ?>
						<div class="memberSelect award-member" data-member-id="<?php echo $member->ID  ;  ?>"
